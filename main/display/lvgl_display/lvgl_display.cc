...
LvglDisplay::~LvglDisplay() {
    ...
    if (encoder_panel_ != nullptr) {
        lv_obj_del(encoder_panel_);
    }
    if (playback_label_ != nullptr) {
        lv_obj_del(playback_label_);
    }
    if (gramo_page_ != nullptr) {
        lv_obj_del(gramo_page_);
    }
...
void LvglDisplay::ShowNotification(const char* notification, int duration_ms) {
    ...
    esp_timer_stop(notification_timer_);
    esp_err_t err = esp_timer_start_once(notification_timer_, duration_ms * 1000);
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "notification timer start failed: %s", esp_err_to_name(err));
    }
}
...
void LvglDisplay::UpdateStatusBar(bool update_all) {
    ...
    // Update time (skip while VMR message is playing so status stays "消息播放中")
    if (app.GetDeviceState() == kDeviceStateIdle && !app.GetVmrClient().IsPlaying()) {
        if (last_status_update_time_ + std::chrono::seconds(10) < std::chrono::system_clock::now()) {
            ...
            // else: system time not set yet — skip status bar clock silently
        }
    }
...
void LvglDisplay::SetEncoderInfo(int volume, int year) {
    DisplayLockGuard lock(this);
    if (encoder_label_ == nullptr) {
        return;
    }
    char buf[32];
    snprintf(buf, sizeof(buf), "%3d   %s %d", volume, FONT_AWESOME_CALENDAR, year);
    lv_label_set_text(encoder_label_, buf);
}

void LvglDisplay::SetPlaybackInfo(const char* info) {
    DisplayLockGuard lock(this);
    if (playback_label_ == nullptr) {
        return;
    }
    if (info && info[0]) {
        lv_label_set_text(playback_label_, info);
        lv_obj_remove_flag(playback_label_, LV_OBJ_FLAG_HIDDEN);
    } else {
...
void LvglDisplay::SetGramophoneInfo(int volume, int year, const char* date, const char* time) {
    DisplayLockGuard lock(this);
    if (gramo_page_ == nullptr) return;

    if (gramo_year_label_) {
        char buf[16];
        snprintf(buf, sizeof(buf), "%d    ", year);
        lv_label_set_text(gramo_year_label_, buf);
    }
    if (gramo_vol_label_) {
        char buf[16];
        snprintf(buf, sizeof(buf), "%d", volume);
        lv_label_set_text(gramo_vol_label_, buf);
    }
    if (gramo_date_label_ && date) {
        lv_label_set_text(gramo_date_label_, date);
    }
    if (gramo_time_label_ && time) {
        lv_label_set_text(gramo_time_label_, time);
    }
}

void LvglDisplay::SetGramophoneOverlay(const char* status, const char* message) {
    DisplayLockGuard lock(this);
    if (gramo_overlay_status_) lv_label_set_text(gramo_overlay_status_, status ? status : "");
    if (gramo_overlay_msg_) lv_label_set_text(gramo_overlay_msg_, message ? message : "");
}
