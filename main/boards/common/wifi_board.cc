...
void WifiBoard::StartNetwork() {
    ...
    config.ssid_prefix = "AI-time-machine";
...
void WifiBoard::OnNetworkEvent(NetworkEvent event, const std::string& data) {
    switch (event) {
        case NetworkEvent::Connected:
            ...
            // Force WiFi power save off to keep connection alive
...
void WifiBoard::SetPowerSaveLevel(PowerSaveLevel level) {
    // VMR queue HTTP stalls under MAX_MODEM PS (connect OK, ReadAll hangs).
    // Ignore LOW_POWER while a VMR wifi hold is active.
    if (level == PowerSaveLevel::LOW_POWER &&
        Application::GetInstance().GetVmrClient().IsWifiHoldActive()) {
        ESP_LOGW(TAG, "Ignore LOW_POWER while VMR Wi-Fi hold active");
